"""Integration tests for OpenSearchAdapter against a real OpenSearch instance."""
...
pytestmark = [pytest.mark.integration, pytest.mark.asyncio, pytest.mark.opensearch]


@pytest.fixture
async def adapter(opensearch_ready):
    a = OpenSearchAdapter(
        hosts=[opensearch_ready],
...
class TestOpenSearchSearch:
    ...
    async def test_search_relevance_federated(self, adapter):
        results = await adapter.search("federated learning medical privacy", SearchOptions(max_results=10))
        titles = [d["_source"]["title"] for d in results.documents]
        assert any("Federated" in t for t in titles)
...
class TestOpenSearchFetchDocument:
    async def test_fetch_existing_document(self, adapter):
        doc = await adapter.fetch_document("doc-002")
        assert doc["_source"]["title"] == "Transformer Models for Natural Language Understanding"
...
class TestOpenSearchSchemaMapping:
    async def test_map_to_standard_schema(self, adapter):
        results = await adapter.search("robotic", SearchOptions(max_results=1))
        assert len(results.documents) > 0
    ...
    async def test_search_and_normalize(self, adapter):
        docs = await adapter.search_and_normalize("graph neural", SearchOptions(max_results=5))
        assert len(docs) > 0
