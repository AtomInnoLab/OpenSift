...
Connects to Elasticsearch clusters (v8+) and provides search,
document retrieval, and health monitoring through the standard
OpenSift adapter interface.
"""
...
class ElasticsearchAdapter(SearchAdapter):
    """Search adapter for Elasticsearch (v8+).

    Supports:
      - Full-text search with BM25
      - Vector/hybrid search (when configured)
      - Aggregations
      - Highlighting
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            logger.info(
                "Connected to Elasticsearch cluster: %s (v%s)",
    ...
    async def search(self, query: str, options: SearchOptions) -> RawResults:
        ...
        try:
            ...
            documents = [hit for hit in hits.get("hits", [])]

            return RawResults(
                total_hits=total,
                documents=documents,
                metadata={"took_es_ms": response.get("took", 0)},
                took_ms=took_ms,
            )
        except Exception as e:
            raise QueryError(f"Elasticsearch query failed: {e}") from e
    ...
    def map_to_standard_schema(self, raw_result: dict[str, Any]) -> StandardDocument:
        """Map an Elasticsearch hit to StandardDocument."""
        ...
        return StandardDocument(
            ...
            metadata=DocumentMetadata(
                ...
                extra={"es_index": raw_result.get("_index")},
            ),
        )
    ...
    def _parse_recency_filter(recency: str) -> dict[str, Any] | None:
        """Parse a recency filter string (e.g., '1y', '6m', '30d') to ES range query."""
        ...
        return {
            "range": {
                "@timestamp": {
                    "gte": f"now-{value}{recency[-1]}/{es_unit}",
                }
            }
        }
